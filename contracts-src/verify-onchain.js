...
//   • Owners of QuickSwap (if deployed) / StepSubscription / StepNFTTreasury
...
//   (optional)  RPC_URL=https://your-node  QUICKSWAP=0x...  node contracts-src/verify-onchain.js
// ─────────────────────────────────────────────────────────────────────────────
...
const A = {
  ...
  QUICKSWAP:     process.env.QUICKSWAP || '', // custom fee-skim proxy, if deployed
};
...
(async () => {
  ...
  if (A.QUICKSWAP) {
    const qs = c(A.QUICKSWAP, ABI.quickswap);
    console.log(`    QuickSwap.owner           : ${label(await safe(() => qs.owner()))}`);
    console.log(`    QuickSwap.feeRecipient    : ${label(await safe(() => qs.feeRecipient()))}`);
    console.log(`    QuickSwap.feeBps          : ${await safe(() => qs.feeBps())}`);
  } else {
    console.log('    QuickSwap (fee-skim proxy): address not provided — set QUICKSWAP=0x... to check, or it is not deployed.');
  }
